...
export function computeScreenSpaceError(
	...
	let sse = (geometricError * screenHeight) / 
	...
	if (distanceFactor !== 1.0) {
		...
		const distanceAdjustment = 1.0 - (normalizedDistance * (1.0 - distanceFactor));
...
/**
 * Determines if a node should be visible based on its distance from the camera
 * and a maximum viewing distance.
 * 
 * @param cameraCenter - The camera position in world space
 * @param center - The center of the node in world space
 * @param maxDistance - Maximum distance at which nodes should be visible
